...
def item(url, medio="A", horas=1, artista="Wet Leg", categoria="gira", relevancia="general", indie=True):
    return {
        "titulo": f"t {url}", "url": url, "url_norm": url, "fecha": AHORA - timedelta(hours=horas),
        "resumen": "r", "medio": medio,
...
def cand(artista, relevancia, fuentes=1, horas=1, indie=True):
    return {"artista": artista, "relevancia": relevancia, "fuentes": [{}] * fuentes,
